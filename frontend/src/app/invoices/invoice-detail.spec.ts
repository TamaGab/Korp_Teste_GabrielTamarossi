...
function mockPrintFrame() {
  ...
  return {
    ...
    dismissPrintDialog: () => {
      if (!afterPrint) {
        throw new Error('afterprint listener was not registered');
      }
...
describe('InvoiceDetail', () => {
  ...
  it('shows an empty state when an Invoice has no Product lines', () => {
    ...
    expect(fixture.nativeElement.textContent).toContain('Nenhum produto nesta nota fiscal');
  ...
  it('shows Portuguese not-found feedback for a missing Invoice', () => {
    ...
    expect(fixture.nativeElement.textContent).toContain('Nota fiscal não encontrada.');
  ...
  it('opens and prints the prepared HTML for an Open Invoice', () => {
    const { dismissPrintDialog, printFrame, printWindow } = mockPrintFrame();
  ...
  it('closes the Invoice when print returns without an afterprint event', () => {
    const { printFrame } = mockPrintFrame();
    const fixture = TestBed.createComponent(InvoiceDetail);
    fixture.detectChanges();
    const http = TestBed.inject(HttpTestingController);
    http.expectOne('http://localhost:8082/invoices/0001').flush({
    ...
    const button = fixture.nativeElement.querySelector('button') as HTMLButtonElement;
    expect(button.disabled).toBe(true);
    expect(button.textContent).toContain('Finalizando nota fiscal...');
  ...
  it('shows every stock problem in Portuguese without opening a window', () => {
    ...
    (fixture.nativeElement.querySelector('button') as HTMLButtonElement).click();
    http.expectOne('http://localhost:8082/invoices/0001/prepare-print').flush(
      {
